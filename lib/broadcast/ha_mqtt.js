...
const broadcasterFactory = (target) => {
    ...
    broadcaster.send = (tag) => {
        ...
        for (const measure of tag.measures) {
            ...
            // TODO: check new measure !
            let value = measure.value
            if (measuresIndex[`${measure.field}`]) {
                value = measure.value.toFixed(measuresIndex[`${measure.field}`].accuracy)
            }
            ...
            if (measuresIndex[`${measure.field}`] && measuresIndex[`${measure.measure}`].unit) {
