...
const validate = () => {
    ...
    if (config.sampling.history === undefined) {
        ...
        config.sampling.history = parseInt(config.sampling.history)
    }
    if (config.sampling.interval === undefined) {
        config.sampling.interval = 10000
    } else {
        config.sampling.interval = parseInt(config.sampling.interval)
    }
    ...
    if (config.battery.min === undefined) {
        ...
        config.battery.min = parseInt(config.battery.min)
    }
    if (config.battery.max === undefined) {
        config.battery.max = 3000
    } else {
        config.battery.max = parseInt(config.battery.max)
    }
