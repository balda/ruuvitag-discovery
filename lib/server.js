...
const hassioConfig = hassio.options()
for (const option in hassioConfig) {
    serverConfig[option] = hassioConfig[option]
}
...
module.exports = {
    start: async ({store, config, addon}) => {
        ...
        if (serverConfig.ssl) {
            ...
            }, app).listen(addon.ingress_port, () => {
                console.log(`Start https web server on port ${addon.ingress_port}`)
            })
        } else {
            server = http.createServer(app).listen(addon.ingress_port, () => {
                console.log(`Start http web server on port ${addon.ingress_port}`)
            })
        }
    ...
    error: (error) => {
        console.log(`Connection Error with "${error.target ? error.target.name : `unknown`}" target: ${error.error}`)
        // socketSend({error})
    },
